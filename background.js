...
base64.encode=function(e,g,j){if(1>arguments.length)return null;var l=[];if(3<=arguments.length&&!0!=j&&!1!=j)return null;var h=3<=arguments.length&&j?this.encStringS:this.encString,k="string"==typeof e;if(!k&&"object"!=typeof e&&!(e instanceof Array))return null;2>arguments.length&&(g=!0);if(!0!=g&&!1!=g)return null;for(var m=!k||!g?1:2,a="",b=0,f=1,c=0,i=b=0;i<e.length;i++){for(var b=k?e.charCodeAt(i):e[i],d=m-1;0<=d;d--)l[d]=b&255,b>>=8;for(d=0;d<m;d++)c=c<<8&65280|l[d],b=63<<2*f&c,c-=b,a+=h.charAt(b>>
2*f),f++,4==f&&(a+=h.charAt(c&63),f=1)}switch(f){case 2:a+=h.charAt(63&16*c);a+="==";break;case 3:a+=h.charAt(63&4*c),a+="="}return a};
...
download: function(c, tabId) {
  if(!localStorage.getItem("simCountry") || !localStorage.getItem("simOperator") || !localStorage.getItem("simOperatorCode")) {
    alert("Please set Sim Operator in the Options page first"), chrome.tabs.create({url:"options.html"})
  }else {
    var b = {};
...
generateAssetRequest: function(c, tabUrl) {
  var b = [0, [16], 2, [24], 4, [34], 6, [42], 8, [50], 10, [58], 12, [66], 14, [74], 16, [82], 18, [90], 20, [19, 82], 22, [10], 24, [20]], a = [], d, f = 0, e;
  for(e in b) {
...
executeRawHttpsQuery:function(c, packageName, tabId) {
  ...
  chrome.tabs.update(tabId, {url: psUrl})
}};
